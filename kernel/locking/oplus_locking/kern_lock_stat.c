	...
	for (i = 0; i < LOCK_TYPES; i++) {
		...
		list_for_each_entry(info, &fatal_record[i].fatal_head, node) {
			ret = snprintf(&buf[idx], (FATAL_SHOW_MAX_BUF - idx),
				"[stamp:%llu,wait_time:%llu,wait_type:%d,waiter_tid:%d,waiter_pid:%d,"
...
static const struct proc_ops fatal_stat_fops = {
	.proc_open		= fatal_stats_open,
	.proc_read		= seq_read,
...
static int hash_insert_node(struct top_node *n)
{
	...
	if (atomic_read(&tinfo.total_cnt[n->grp_idx]) >= MAX_NODE_IN_HASH_PER_GRP) {
		kfree(n);
		lk_err("[kern_lock_stat] : Top node reach max limit, ignore it.\n");
		return 0;
	}
...
static const struct proc_ops top_stat_fops = {
	.proc_open		= top_stats_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release		= single_release,
};
...
=======
>>>>>>> 815cbb9920c03095405698e95493b5355e17b1fa
...
static __always_inline void lk_contended(int type)
{
	...
	if (unlikely(IS_ERR_OR_NULL(ots))) {
		lk_err("kern_lock_stat : error start, but ots == NULL !\n");
		return;
	}

	if (ots->lkinfo.waittime_stamp) {
		lk_err("kern_lock_stat : error start,times_stamp not 0,type = %d !\n", type);
		return;
	}

	ots->lkinfo.waittime_stamp = lockstat_clock();
}
...
static __always_inline void lk_acquired(int type)
{
	...
	if (unlikely(IS_ERR_OR_NULL(ots))) {
		lk_err("kern_lock_stat : erro end, but ots == NULL !\n");
		return;
	}
	if (ots->lkinfo.waittime_stamp) {
		now = lockstat_clock();
		delta = now - ots->lkinfo.waittime_stamp;
		ots->lkinfo.waittime_stamp = 0;
		handle_wait_stats(type, delta);
	} else {
		lk_err("kern_lock_stat : error end,"
...
static void android_vh_rwsem_read_wait_start_handler(void *unused, struct rw_semaphore *sem)
{
