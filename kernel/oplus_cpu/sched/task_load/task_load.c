...
static void update_runtime_handler(void *data, bool preempt, struct task_struct *prev, struct task_struct *next, unsigned int prev_state)
{
	u64 normalize_delta = 0;
	u64 delta = 0;
	...
	if (!ots_prev->is_update_runtime) {
		...
		if (prev->se.sum_exec_runtime > ots_prev->exec_calc_runtime) {
			...
			normalize_delta = scale_exec_time(delta, cpu_rq(task_cpu(prev)));
			all_running_time += delta;
			normalize_all_running_time += normalize_delta;
