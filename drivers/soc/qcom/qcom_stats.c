...
static struct qcom_stats_data *drv_backup;
#endif
				...
				struct kobj_attribute *attr, char *buf)
{
	...
	struct sleep_stats *stat;
	const char *name;
	struct qcom_stats_data *stats_drv = container_of(attr,
					   struct qcom_stats_data, ka_master_oplus);
...
static int soc_sleep_stats_create_sysfs(struct platform_device *pdev,
					struct qcom_stats_data *stats_drv)
{
	int ret = 0;
	...
	stats_drv->stat_kobj = kobject_create_and_add("soc_sleep", oplus_power_kobj);
	if (!stats_drv->stat_kobj)
		return -ENOMEM;

	sysfs_attr_init(&stats_drv->ka_stat_oplus.attr);
	stats_drv->ka_stat_oplus.attr.mode = 0444;
	stats_drv->ka_stat_oplus.attr.name = "oplus_rpmh_stats";
	stats_drv->ka_stat_oplus.show = oplus_rpmh_stats_show;

	ret = sysfs_create_file(stats_drv->stat_kobj, &stats_drv->ka_stat_oplus.attr);

	stats_drv->master_kobj = kobject_create_and_add("rpmh_stats", oplus_power_kobj);
	if (!stats_drv->master_kobj)
		return -ENOMEM;

	sysfs_attr_init(&stats_drv->ka_master_oplus.attr);
	stats_drv->ka_master_oplus.attr.mode = 0444;
	stats_drv->ka_master_oplus.attr.name = "oplus_rpmh_master_stats";
	stats_drv->ka_master_oplus.show = oplus_rpmh_master_stats_show;

	ret |= sysfs_create_file(stats_drv->master_kobj, &stats_drv->ka_master_oplus.attr);
...
static int qcom_stats_probe(struct platform_device *pdev)
{
	...
	stats_drv = devm_kzalloc(&pdev->dev, sizeof(*stats_drv), GFP_KERNEL);
	if (!stats_drv)
		return -ENOMEM;
#endif
...
#ifdef CONFIG_OPLUS_POWERINFO_RPMH
	stats_drv->node = pdev->dev.of_node;
#endif
...
#ifdef CONFIG_OPLUS_POWERINFO_RPMH
	soc_sleep_stats_create_sysfs(pdev, stats_drv);
	drv_backup = stats_drv;
#endif
